...
namespace TabBlazor
{
    public partial class CustomDropdownItem : TablerBaseComponent, IDisposable
    {
        [CascadingParameter(Name = "Dropdown")] public CustomDropdown Dropdown { get; set; }
        [CascadingParameter(Name = "DropdownMenu")] public CustomDropdownMenu ParentMenu { get; set; }
        ...
        private string GetWrapperClass()
        {
            if (hasSubMenu)
            {
                if(Dropdown.SubMenusDirection == CustomDropdownDirection.Down)
                {
                    return "dropdown";
                }
                else
